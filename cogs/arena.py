...
class Arena(commands.Cog):
    ...
    async def arena_start(self, ctx):
        logging.debug(f"arena game attempted by {str(ctx.author)}!")
        if self.arena_ongoing:
            await ctx.reply("arena game already ongoing. use $arenaJoin to join the game, or $arenaFight to conclude the ongoing game.")
            return

        self.arena_players.append(ctx.author)
        self.arena_ongoing = True
        await ctx.reply(str(ctx.author)+" started an arena game! type $arenaJoin to join.")
    ...
    async def arena_fight(self, ctx):
        if not self.arena_ongoing:
            ctx.reply("No arena started. use $arena_start.")
            return
